...
class ActivationSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if not User.objects.filter(email=email, activation_code=code).exists(): # exists -> проверяет на наличие обьектов (если существует True, иначе False)
...
class ChangePasswordSerializer(serializers.Serializer):
    old_password = serializers.CharField(min_length=1, required=True)
    ...
    def validate(self, attrs):
        ...
        if old_pass == new_pass1:
            raise serializers.ValidationError('Пароли совподают')
        return attrs
